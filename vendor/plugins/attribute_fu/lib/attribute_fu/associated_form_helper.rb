module AttributeFu
  ...
  module AssociatedFormHelper
    ...
    def add_associated_link(name, object, opts = {})
      ...
      function << "$(#{container}).append($.template("+[self.render_associated_form(object, :fields_for => { :javascript => true }, :partial => partial)].flatten.first.to_json+"), { number: --#{variable}});"
      function << "$(#{container}).zebra()"
